...
export default function CompareDrawer() {
  ...
  return (
    <div className="fixed bottom-20 sm:bottom-6 left-1/2 -translate-x-1/2 z-[90] w-[94%] max-w-2xl bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl border border-orange-100 dark:border-gray-800 rounded-[24px] shadow-[0_20px_50px_rgba(249,115,22,0.18)] px-4 py-3 sm:px-5 sm:py-3.5 transition-all duration-300 transform translate-y-0 flex flex-col gap-1.5 animate-[slideUp_0.4s_ease-out]">
      <style jsx global>{`
        @keyframes slideUp {
          from {
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 overflow-x-auto py-1 scrollbar-none flex-1">
          ...
          {/* Add placeholder slots if less than 2 items */}
          {items.length < 2 && (
            <>
              ...
              <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl border border-dashed border-gray-200 flex items-center justify-center text-base text-gray-300 select-none bg-gray-50/50">
                +
              </div>
            </>
          )}
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          {items.length >= 2 ? (
            ...
            >
              ...
              <span className="bg-white/20 text-white px-1.5 py-0.5 rounded-md text-[9px] sm:text-xs font-black leading-none">
                {items.length}
              </span>
            </Link>
          ) : (
          ...
          <button
            onClick={handleClear}
            className="text-gray-400 hover:text-red-500 transition-colors p-1.5 sm:p-2 text-xs font-semibold hover:bg-gray-50 dark:hover:bg-gray-800 rounded-xl"
